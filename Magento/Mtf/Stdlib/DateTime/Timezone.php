...
class Timezone implements \Magento\Mtf\Stdlib\DateTime\TimezoneInterface
{
    /**
     * @var array
     */
    protected $_allowedFormats = array(
        \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_FULL,
        \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_LONG,
        \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_MEDIUM,
        \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT
    );
    ...
    /**
     * @var \Magento\Mtf\Stdlib\DateTime
     */
    protected $_dateTime;

    /**
     * @var \Magento\Mtf\Stdlib\DateTime\DateFactory
     */
     ...
     * @param \Magento\Mtf\Stdlib\DateTime $dateTime
    ...
    public function __construct(
        ...
        \Magento\Mtf\Stdlib\DateTime $dateTime,
        \Magento\Mtf\Stdlib\DateTime\DateFactory $dateFactory,
        $defaultTimezonePath
    ) {
    ...
    {
        return \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::DEFAULT_TIMEZONE;
    }
    ...
    public function getDateFormatWithLongYear()
    {
        return preg_replace(
            '/(?<!y)yy(?!y)/',
            'yyyy',
            $this->_getTranslation(\Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT, 'date')
        );
    }
    ...
    public function date($date = null, $part = null, $locale = null, $useTimezone = true)
    {
        ...
        if (empty($date)) {
            // $date may be false, but \Magento\Mtf\Stdlib\DateTime\DateInterface uses strict compare
            $date = null;
        }
    ...
    public function formatDate(
        $date = null,
        $format = \Magento\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT,
        $showTime = false
    ) {
        if (!in_array($format, $this->_allowedFormats, true)) {
            return $date;
        }
        if (!$date instanceof \Magento\Mtf\Stdlib\DateTime\DateInterface && $date && !strtotime($date)) {
            return '';
        }
        if (is_null($date)) {
            $date = $this->date(gmdate('U'), null, null);
        } elseif (!$date instanceof \Magento\Mtf\Stdlib\DateTime\DateInterface) {
    ...
    public function formatTime(
        ...
        if (is_null($time)) {
            $date = $this->date(time());
        } elseif ($time instanceof \Magento\Mtf\Stdlib\DateTime\DateInterface) {
            $date = $time;
        } else {
    ...
    public function utcDate($scope, $date, $includeTime = false, $format = null)
    {
        ...
        $dateObj->setTimezone(\Magento\Mtf\Stdlib\DateTime\TimezoneInterface::DEFAULT_TIMEZONE);
        return $dateObj;
    }
