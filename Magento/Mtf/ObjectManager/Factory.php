...
use Magento\Mtf\System\Code\ClassReader;
 ...
 * @internal
 */
class Factory extends \Magento\Mtf\ObjectManager\Factory\Dynamic\Developer
{
    /**
     * @var \Magento\Mtf\System\Code\ClassReader
     */
    ...
    public function create($requestedType, array $arguments = array())
    {
        ...
        if ($parameters == null) {
            if (false !== strpos($type, 'Magento\Framework\ObjectManagerInterface')) {
                throw new \Exception(implode('FFFF', debug_backtrace()));
            }
            return new $type();
        }
