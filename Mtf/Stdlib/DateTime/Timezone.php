...
class Timezone implements \Mtf\Stdlib\DateTime\TimezoneInterface
{
    /**
     * @var array
     */
    protected $_allowedFormats = array(
        \Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_FULL,
        \Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_LONG,
        \Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_MEDIUM,
        \Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT
    );
    ...
    /**
     * @var \Mtf\Stdlib\DateTime
     */
    protected $_dateTime;

    /**
     * @var \Mtf\Stdlib\DateTime\DateFactory
     */
     ...
     * @param \Mtf\Stdlib\DateTime $dateTime
    ...
    public function __construct(
        ...
        \Mtf\Stdlib\DateTime $dateTime,
        \Mtf\Stdlib\DateTime\DateFactory $dateFactory,
        $defaultTimezonePath
    ) {
    ...
    {
        return \Mtf\Stdlib\DateTime\TimezoneInterface::DEFAULT_TIMEZONE;
    }
    ...
    public function getDateFormatWithLongYear()
    {
        return preg_replace(
            '/(?<!y)yy(?!y)/',
            'yyyy',
            $this->_getTranslation(\Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT, 'date')
        );
    }
    ...
    public function date($date = null, $part = null, $locale = null, $useTimezone = true)
    {
        ...
        if (empty($date)) {
            // $date may be false, but \Mtf\Stdlib\DateTime\DateInterface uses strict compare
            $date = null;
        }
    ...
    public function formatDate(
        $date = null,
        $format = \Mtf\Stdlib\DateTime\TimezoneInterface::FORMAT_TYPE_SHORT,
        $showTime = false
    ) {
        if (!in_array($format, $this->_allowedFormats, true)) {
            return $date;
        }
        if (!$date instanceof \Mtf\Stdlib\DateTime\DateInterface && $date && !strtotime($date)) {
            return '';
        }
        if (is_null($date)) {
            $date = $this->date(gmdate('U'), null, null);
        } elseif (!$date instanceof \Mtf\Stdlib\DateTime\DateInterface) {
    ...
    public function formatTime(
        ...
        if (is_null($time)) {
            $date = $this->date(time());
        } elseif ($time instanceof \Mtf\Stdlib\DateTime\DateInterface) {
            $date = $time;
        } else {
    ...
    public function utcDate($scope, $date, $includeTime = false, $format = null)
    {
        ...
        $dateObj->setTimezone(\Mtf\Stdlib\DateTime\TimezoneInterface::DEFAULT_TIMEZONE);
        return $dateObj;
    }
